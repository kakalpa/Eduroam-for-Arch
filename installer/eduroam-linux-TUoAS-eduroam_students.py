...
parser.add_argument("--install-iwd", action='store_true', dest='install_iwd',
                    help='generate iwd config files and install them into /var/lib/iwd')
...
# Suppress dbus debug logging noise
try:
    import logging
...
def run_installer() -> None:
    ...
    if hasattr(ARGS, 'install_iwd') and ARGS.install_iwd:
        install_iwd = ARGS.install_iwd

    def detect_iwd() -> bool:
        """Detect a likely iwd installation/environment.

        Heuristics:
        - presence of system iwd config dir `/var/lib/iwd`
        - `iwd` binary on PATH
        """
        try:
            if os.path.isdir('/var/lib/iwd'):
                return True
            if shutil.which('iwd') is not None:
                return True
        except Exception:
            return False
        return False
    ...
    def auto_select_backend() -> str:
        """
        Automatically detect and select the best available backend.
        
        Priority order:
        1. NetworkManager (if DBus available and responsive)
        2. iwd (if present and running)
        3. wpa_supplicant (fallback - works on all systems)
        
        Returns: 'nm', 'iwd', or 'wpa'
        """
        global NM_AVAILABLE
        ...
        if NM_AVAILABLE:
            try:
                ...
                if config_tool.connect_to_nm() is not None:
                    debug("Backend selection: NetworkManager detected and available")
                    return 'nm'
            except Exception as e:
                debug(f"NetworkManager check failed: {e}")
        
        NM_AVAILABLE = False
        
        # Try iwd second
        if detect_iwd():
            debug("Backend selection: iwd detected and available")
            return 'iwd'
        
        # Fall back to wpa_supplicant (always available)
        debug("Backend selection: Using wpa_supplicant (fallback)")
        return 'wpa'
    ...
    # User can still explicitly override with flags
    if wpa_conf:
        backend = 'wpa'
    elif iwd_conf:
        backend = 'iwd'
    elif install_iwd:
        backend = 'iwd'  # iwd_conf and install_iwd both use iwd backend
    else:
        # Auto-detect best backend for the system
        backend = auto_select_backend()

    # Now install based on selected backend
    installer_data.get_user_cred()
    installer_data.save_ca()

    if backend == 'nm':
        debug("Installing via NetworkManager")
        config_tool = CatNMConfigTool()
        ...
        # If --install-iwd was specified, also install to system
        if install_iwd:
            debug("Installing iwd configs to /var/lib/iwd")
            for ssid in Config.ssids:
                # create the appropriate config in-memory (reuse the existing private creators)
                if Config.eap_outer == 'PWD':
                    iwd_config._create_eap_pwd_config(ssid, installer_data)
                elif Config.eap_outer == 'PEAP':
                    iwd_config._create_eap_peap_config(ssid, installer_data)
                elif Config.eap_outer == 'TTLS':
                    iwd_config._create_ttls_pap_config(ssid, installer_data)
                else:
                    raise ValueError('Invalid connection type')
                iwd_config.install_config(ssid)
    else:  # backend == 'wpa'
        debug("Installing via wpa_supplicant")
        wpa_config = WpaConf()
...
class IwdConfiguration:
    ...
    def install_config(self, ssid: str, dest_dir: str = '/var/lib/iwd') -> None:
        """
        Install the generated config into the system iwd directory. This
        writes a temporary file and attempts to move it into place; if a
        permission error occurs it will call `pkexec` to perform the install
        atomically.
        """
        dest_path = os.path.join(dest_dir, f"{ssid}.8021x")
        # write a temp file in a writable location
        tmp_path = os.path.join('/tmp', f'{ssid}.8021x.{os.getpid()}')
        try:
            with open(tmp_path, 'w') as tf:
                tf.write(self.config)
            try:
                copyfile(tmp_path, dest_path)
                debug(f'Installed iwd config to {dest_path}')
            except PermissionError:
                # use pkexec+install to move the file into place with correct perms
                cmd = f"install -m 644 '{tmp_path}' '{dest_path}'"
                subprocess.run(['pkexec', 'bash', '-c', cmd], check=True)
                debug(f'Installed iwd config to {dest_path} via pkexec')
            # Ensure file is owned by root and has correct permissions so iwd daemon can read it
            try:
                # Try to fix perms directly (works if user is root)
                os.chmod(dest_path, 0o644)
                debug(f'Set permissions 644 on {dest_path}')
            except PermissionError:
                # File is owned by root; use pkexec to fix perms
                cmd = f"chmod 644 '{dest_path}' && chown root:root '{dest_path}'"
                try:
                    subprocess.run(['pkexec', 'bash', '-c', cmd], check=True)
                    debug(f'Fixed ownership/permissions on {dest_path} via pkexec')
                except subprocess.CalledProcessError:
                    print(f'WARNING: Could not fix permissions on {dest_path}. iwd may not be able to read the config.')
        finally:
            try:
                os.remove(tmp_path)
            except Exception:
...
Messages.cat_dir_exists = "Directory {} exists; some of its files may " \
    "be overwritten"
Config.instname = "Turku University of Applied Sciences"
Config.profilename = "eduroam students"
Config.url = "your local eduroam® support page"
Config.email = "servicedesk@turkuamk.fi"
Config.title = "eduroam CAT"
Config.server_match = "turkuamk.fi"
...
Config.user_realm = "edu.turkuamk.fi"
Config.ssids = ['eduroam']
Config.del_ssids = []
Config.servers = ['DNS:cpass.turkuamk.fi', 'DNS:cpass01.turkuamk.fi', 'DNS:cpass02.turkuamk.fi']
Config.use_other_tls_id = False
Config.anonymous_identity = "anonymous@edu.turkuamk.fi"
